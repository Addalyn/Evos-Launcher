...
const fetchInfo = async (
  ...
  try {
    const strapi = strapiClientv1.from('games').select();
...
const fetchCount = async (
  ...
  try {
    const strapi = strapiClientv1.from('games/count').select();
...
export function Games({
    ...
    const fetchReplayInfo = async (GameServerProcessCode: string) => {
      try {
        const response = await fetch(
          `${strapiClientv1.getApiUrl()}/upload/files?filters[name][$contains]=${GameServerProcessCode}`,
        );
  ...
  const handleOpenReplay = () => {
    ...
    fetchReplay(
      `${strapiClientv1.getApiUrl().replace('/api', '')}${replay?.url}` as string,
    );
  };
